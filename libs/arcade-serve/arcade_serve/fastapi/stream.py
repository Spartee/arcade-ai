...
class StreamComponent(WorkerComponent):
    def __init__(self, worker: Worker, local_context: dict[str, Any] | None = None) -> None:
        ...
        print(f"DEBUG: StreamComponent init - worker.disable_auth = {worker.disable_auth}")
            ...
            try:
                ...
                for session_id, session in sessions_to_close:
                    try:
                        ...
                        logging.getLogger("arcade.mcp").warning(
                ...
                logging.getLogger("arcade.mcp").error(f"Error in session cleanup: {e}")
    ...
    def register(self, router: Router) -> None:
        ...
        print(
            f"DEBUG: StreamComponent.register - self.worker.disable_auth = {self.worker.disable_auth}"
        )
        ...
        if isinstance(router, FastAPIRouter):
            ...
            print(f"DEBUG: StreamComponent.register - dependencies = {dependencies}")
    ...
    async def __call__(self, request: Request) -> StreamingResponse:
        ...
        try:
            ...
            print(f"DEBUG: Read {len(body_bytes)} bytes from request")
        except Exception as e:
            print(f"DEBUG: Error reading request body: {e}")
            return StreamingResponse(
                iter([
                    json.dumps({
                        "error": f"Failed to read request body: {e!s}",
                        "status": "error",
                    })
                    + "\n"
                ]),
            ...
            try:
                while True:
                    try:
                        ...
                        print(f"DEBUG: Got message from queue: {message}")
                        if message is None:  # Sentinel value
                            break
                        yield json.dumps(message) + "\n"
                    except asyncio.TimeoutError:
                        print("DEBUG: Timeout, sending ping")
                ...
                logging.getLogger("arcade.mcp").error(f"Stream error: {e}")
        ...
        async def process_request(body_bytes: bytes):
            print(f"DEBUG: process_request started with {len(body_bytes)} bytes")
            try:
                ...
                try:
                    ...
                    print(f"DEBUG: Got response from MCP server: {response}")
                    if response:
                        ...
                        print(f"DEBUG: Put response in queue: {response_data}")
                    else:
                        print("DEBUG: No response from MCP server")
                except Exception as e:
                    import logging

                    logging.getLogger("arcade.mcp").error(f"Error processing message: {e}")
                    await session.queue.put({
                ...
                logging.getLogger("arcade.mcp").error(
                    f"Unexpected error in request processing: {e}"
                )
                try:
                    await session.queue.put({
