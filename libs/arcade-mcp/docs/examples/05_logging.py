...
async def error_handling_example(
    ...
    try:
        await context.mcp.log("debug", f"Error handling test: should_fail={should_fail}")

        if should_fail:
            if error_type == "ValueError":
                raise ValueError("This is a simulated value error")
            elif error_type == "KeyError":
                raise KeyError("missing_key")
            elif error_type == "ZeroDivisionError":
                result = 1 / 0
            else:
                raise Exception(f"Generic error of type: {error_type}")
...
async def batch_processing_logs(
    ...
    await context.mcp.log("info", f"Starting batch processing of {len(items)} items")
