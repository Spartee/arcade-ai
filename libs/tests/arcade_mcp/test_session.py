...
from arcade_mcp.session import ServerSession, InitializationState
...
class TestServerSession:
    ...
    async def test_message_processing(self, server_session):
        ...
        # Process a message
        await server_session._process_message(
    ...
    async def test_session_run_loop(self, mcp_server, mock_read_stream, mock_write_stream):
        ...
        mcp_server.handle_message = AsyncMock(return_value=JSONRPCResponse(
            jsonrpc="2.0",
            id=1,
            result={}
        ))
