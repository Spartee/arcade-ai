...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("roots/list", None),
        ("completion/complete", {"ref": {"type": "ref/prompt", "name": "x"}, "argument": {"name": "q", "value": ""}}),
    ],
)
async def test_cancel_all_sends_notifications_and_fails_futures(mcp_server, mock_read_stream, mock_write_stream, method, params):
    session = ServerSession(server=mcp_server, read_stream=mock_read_stream, write_stream=mock_write_stream)
    assert session._request_manager is not None

    mock_write_stream.send = AsyncMock()

    pending_task = asyncio.create_task(session._request_manager.send_request(method, params, timeout=5.0))
    ...
    sent_methods = [json.loads(call[0][0].strip()).get("method") for call in mock_write_stream.send.call_args_list]
...
async def test_closed_flag_drops_late_responses(mcp_server, mock_read_stream, mock_write_stream, method, params):
    session = ServerSession(server=mcp_server, read_stream=mock_read_stream, write_stream=mock_write_stream)
    assert session._request_manager is not None

    mock_write_stream.send = AsyncMock()

    send_task = asyncio.create_task(session._request_manager.send_request(method, params, timeout=5.0))
    await asyncio.sleep(0)
...
@pytest.mark.parametrize(
    "method,params",
    [
        ("completion/complete", {"ref": {"type": "ref/prompt", "name": "x"}, "argument": {"name": "q", "value": "v"}}),
        ("sampling/createMessage", {"messages": [], "maxTokens": 1}),
    ],
)
async def test_new_requests_rejected_after_close(mcp_server, mock_read_stream, mock_write_stream, method, params):
    session = ServerSession(server=mcp_server, read_stream=mock_read_stream, write_stream=mock_write_stream)
    assert session._request_manager is not None
